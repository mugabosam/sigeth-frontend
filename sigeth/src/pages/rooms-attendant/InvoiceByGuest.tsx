...
export default function InvoiceByGuest() {
  const { t } = useLang();
  const { paymentModes, currencies } = useHotelData();
      ...
      <div className="bg-white rounded border border-hotel-border p-4">
        ...
        <div className="flex gap-3 flex-wrap">
          <input
            ...
            className="border-2 border-hotel-border rounded px-4 py-3 text-sm w-48 focus:outline-none focus:border-hotel-gold focus:shadow-md transition-colors"
          />
          <input
            value={queryGuest}
            ...
            onKeyDown={(e) => e.key === "Enter" && handleSearch()}
            className="flex-1 border-2 border-hotel-border rounded px-4 py-3 text-sm focus:outline-none focus:border-hotel-gold focus:shadow-md transition-colors"
          />
          <button
                ...
                {(displayData as any)?.tax?.taux != null ? (
                  <>
                    ...
                    <p className="text-hotel-text-secondary">
                      {t("hTVALabel")}: {fmt((displayData as any).tax.htva ?? 0)} {displayData.current_mon}
                    </p>
                    <p className="text-hotel-text-secondary">
                      {t("tVALabel")} ({(displayData as any).tax.taux}%): {fmt((displayData as any).tax.tva ?? 0)} {displayData.current_mon}
                    </p>
                  </>
                ) : (
                  <p className="text-hotel-text-primary">
